...
export function complexToSphere(re,im){if(isNaN(re)||isNaN(im)||!isFinite(re)||!isFinite(im))return{x:NaN,y:NaN,z:NaN};const d=re*re+im*im+1;if(d===0)return{x:NaN,y:NaN,z:NaN};return{x:2*re/d,y:2*im/d,z:(re*re+im*im-1)/d};}
...
export function projectSphereToCanvas2D(p3D_r,sCX,sCY,sR){if(isNaN(p3D_r.x))return{x:NaN,y:NaN,isVisible:false};return{x:sCX+p3D_r.x*sR,y:sCY-p3D_r.y*sR,isVisible:p3D_r.z>-1e-3};} 
